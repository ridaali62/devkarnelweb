...
export default function Testimonials() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      const cols = getCols();
      setCardWidth((containerRef.current.offsetWidth - (cols - 1) * GAP) / cols);
    };
    update();
  ...
  const slide = (direction) => {
    ...
    const cols = getCols();
    const cw = (containerRef.current.offsetWidth - (cols - 1) * GAP) / cols;
    const step = cw + GAP;
    const totalWidth = step * testimonials.length;

    let nextX = xRef.current + (direction === "next" ? -step : step);

    // Going backwards past the start — jump to equivalent position in second set
    if (nextX > 0) {
      gsap.set(sliderRef.current, { x: xRef.current - totalWidth });
      xRef.current -= totalWidth;
      nextX = xRef.current + step;
    }
    ...
    gsap.to(sliderRef.current, {
      ...
      onComplete: () => {
        xRef.current = nextX;
        // Going forward past the end — jump back to first set equivalent
  ...
  // Double the slides so infinite wrap is seamless
